...
[kaggle kernel](https://www.kaggle.com/davidkor/numpy-kafka-sentence-generate-rnn) <- update 2019-01-03
...
def lossFunc(inputs, targets, hprev):
    ...
    # inputs的每个单词，进行RNN运算后，输出一个shape=（vocab，1）的向量，代表输出单词是词表中每个词的概率
    # ---X---inputs: (25, 63, 1), a sentence, contains 25 words, which word has a (vocab,1) shape vector
    # ---X---outputs: (25, 63, 1), the label of input which has the same shape of input
