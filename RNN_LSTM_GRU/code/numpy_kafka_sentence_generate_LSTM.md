...
> **---X---** 走完1次全部corpus是一个iteration，每训练完100个iteration/走完100次全部数据集, 使用训练出来的参数生成一个200字母/汉字的句子并打印，不是100取整的iteration时，不执行打印，进行下面步骤

> 每训练完一个句子（25词）为一个iteration，每训练100个句子/iteration，根据训练参数生成一个200个单词的句子并打印，同时画出loss曲线
